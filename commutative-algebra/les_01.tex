...
\begin{definition}[Ring]
    A \term{ring} \( A \) is a set with two binary operations (addition and multiplication) such that:
    \begin{enumerate}
...
x = x1 = x0 = 0,
\]
and so \( A \) has only one element, \( 0 \). In this case, \( A \) is the \term{zero ring}, denoted by \( 0 \).
\end{remark}
    
\begin{definition}[Ring Homomorphism]
A \term{ring homomorphism} is a mapping \( f: A \to B \) between rings \( A \) and \( B \) such that:
\begin{enumerate}
    \item \( f(x + y) = f(x) + f(y) \),
    \item \( f(xy) = f(x)f(y) \),
    \item \( f(1) = 1 \).
...
\term{subring} if it is closed under addition and multiplication and
...
\begin{definition}[Ideal]
    An \term{ideal} \( \mathfrak{a} \subseteq A \) is a subset which is an additive subgroup and satisfies \( A\mathfrak{a} \subseteq \mathfrak{a} \), i.e., for all \( x \in A \), \( y \in \mathfrak{a} \), we have \( xy \in \mathfrak{a} \).
    \end{definition}
    
    \begin{definition}[Quotient Ring]
    Given a ring \( A \) and an ideal \( \mathfrak{a} \subset A \), the \term{quotient ring} \( A/\mathfrak{a} \) is the additive quotient group equipped with multiplication induced from \( A \), where the elements are cosets of \( \mathfrak{a} \), and the mapping
    \[
...
\leavevmode\vspace{0.5em} % forces line break after "Remark."

\textbf{Remarks.}

\begin{enumerate}
    \item If \( A \) is Noetherian (Chapter 7), we can avoid the use of Zorn’s lemma: the set of all ideals \( \ne (1) \) has a maximal element.
    \item There exist rings with exactly one maximal ideal, for example: local rings. A ring \( A \) with exactly one maximal ideal is called a \term{local ring}. The field \( k = A / \mathfrak{m} \) is called the \term{residue field} of \( A \).
\end{enumerate}
...
\begin{prop}
    ...
    \end{enumerate}
\end{prop}
A ring with only a finite number of maximal ideals is called \term{semi-local}.
...
\begin{eg}
A \term{principal ideal domain} is an integral domain in which every ideal is principal. In such a ring, every non-zero prime ideal is maximal.
...
The ideal $\mathfrak{N}$ is called the \term{nilradical} of $A$. An equivalent definition is the following:

\begin{prop}
The nilradical of $A$ is the intersection of all the prime ideals of $A$.
\end{prop}

The \term{Jacobson radical} $\mathfrak{R}$ of $A$ is defined to be the intersection of all the maximal ideals of $A$. It can be characterized as follows:
...
More generally, for any (possibly infinite) family of ideals \( \mathfrak{a}_i \) of \( A \), we define the sum \( \sum_{i \in I} \mathfrak{a}_i \) to be the set of all finite sums \( \sum x_i \), where \( x_i \in \mathfrak{a}_i \) for all \( i \in I \), and almost all \( x_i = 0 \).
...
% \section{Prime ideals & Maximal Ideals}
